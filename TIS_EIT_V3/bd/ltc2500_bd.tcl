...
proc create_root_design { parentCell } {
  ...
  # NOTE (hand-patch): TIS sequencer I/O.
  # btnu (B19) = start a bounded measurement run; debounced inside IP_1.
  # nerve_pulse -> PMOD JD pin 1 (V27), drives the switched-resistor nerve phantom.
  set btnu [ create_bd_port -dir I btnu ]
  set nerve_pulse [ create_bd_port -dir O nerve_pulse ]
  ...
  # NOTE (hand-patch): xlconstant_2 removed. It used to tie addr_gen_0/dds_sync
  # low. dds_sync is now a real signal again — IP_1_0/dds_hold (= ~tis_on) drives
  # it as a LEVEL-sensitive "hold A/B phase at 0" during the TIS-off window.
  # See IP_1_0_dds_hold below.
  ...
  # NOTE (hand-patch): IP_1/sync now pulses once per 200 ms stimulation boundary
  # (it used to mark channel-C sine wraps). Still an ILA marker ONLY — the DDS
  # phase hold is a separate, level-sensitive signal, IP_1/dds_hold, see below.
  ...
  # NOTE (hand-patch): TIS sequencer nets, all sourced from IP_1_0.
  # dds_hold = ~tis_on -> addr_gen/dds_sync holds the A/B (F1/F2) phase
  # accumulators at 0 for the whole TIS-off window, so every burst starts from an
  # identical phase. addr_gen C/D are untouched.
  connect_bd_net -net IP_1_0_dds_hold  [get_bd_pins IP_1_0/dds_hold] \
  [get_bd_pins addr_gen_0/dds_sync]
  # tis_on gates the FSM's A/B DAC gain to zero (mid-scale = no stimulation) and
  # tags every ADC sample (header byte bit7) at the MCLK sampling aperture.
  connect_bd_net -net IP_1_0_tis_on  [get_bd_pins IP_1_0/tis_on] \
  [get_bd_pins FSM_0/tis_on] \
  [get_bd_pins ethernet_debug_0/tis_on] \
  [get_bd_pins ila_0/probe36]
  # run_active tags samples belonging to a button-started run (header byte bit3).
  connect_bd_net -net IP_1_0_run_active  [get_bd_pins IP_1_0/run_active] \
  [get_bd_pins ethernet_debug_0/run_active] \
  [get_bd_pins ila_0/probe37]
  # Nerve-phantom pulse -> PMOD JD1 (drives the switched-resistor phantom board).
  connect_bd_net -net IP_1_0_nerve_pulse  [get_bd_pins IP_1_0/nerve_pulse] \
  [get_bd_ports nerve_pulse] \
  [get_bd_pins ila_0/probe38]
  # BTNU (B19) starts a bounded measurement run. Debounced + edge-detected inside
  # IP_1 (hardware, no software polling).
  connect_bd_net -net btnu_1  [get_bd_ports btnu] \
  [get_bd_pins IP_1_0/btn_start]

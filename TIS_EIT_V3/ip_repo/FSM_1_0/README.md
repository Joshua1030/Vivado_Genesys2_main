...
| `0x08` | slv_reg2 | `[15:0]` | **Channel A amplitude gain**, Q12 (4096 = ×1.0). Scales only the AC swing around the 0x8000 common-mode code: `out = 0x8000 + (sine − 0x8000)·gain >> 12`, saturated to `[0, 65535]`. **`0` selects unity gain**, not silence. |
| `0x0C` | slv_reg3 | `[15:0]` | Channel B amplitude gain, same format |
| `0x10` | slv_reg4 | `[15:0]` | Channel C amplitude gain, same format |
| `0x14` | slv_reg5 | `[15:0]` | Channel D amplitude gain, same format |
| `0x18`–`0x3C` | slv_reg6..15 | — | unused |

### TIS burst gating — the `tis_on` port

Channels **A and B** carry the two TIS carriers (F1/F2) and must go silent
between bursts. That is done in hardware, not software, through the `tis_on`
input (driven by [`IP_1`](../IP_1_1_0/README.md)):

- `tis_on = 1` → A/B use their `slv_reg2`/`slv_reg3` gain normally.
- `tis_on = 0` → A/B gain is forced to **0**, so `dac_word` becomes exactly
  `0x8000` (common-mode, no stimulation current).

Channels C and D ignore `tis_on` entirely.

Two reasons this is a port and not a register write: the gain registers cannot
express silence (`0` means unity, see above), and a 100 ms-scale burst envelope
has no business being driven by the MicroBlaze poking registers. `IP_1` also
parks the A/B DDS phase at 0 over the same window via `addr_gen/dds_sync`, so
every burst starts from an identical waveform.
...
Standalone XSim (fast, from `TIS_EIT_V3/`):

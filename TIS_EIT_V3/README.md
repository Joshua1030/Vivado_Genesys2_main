# TIS_EIT_V3 — LTC2500 EIT Acquisition System (Genesys2)
...
(or open the Vivado GUI and run `source .../TIS_EIT_V3/scripts/recreate_project.tcl`
...
to skip the hardware build, copy the XSA into `TIS_EIT_V3/` and commit it — the
...
- **Press BTNU (B19) to start a measurement.** Nothing stimulates until you do:
  at reset the TIS carriers are muted and their DDS phase parked at 0. One press
  runs `frame_count` frames (default 10) and then returns to idle. The button is
  debounced and edge-detected in hardware inside `IP_1` — no software polling —
  so a single press is a single run. The ADC keeps converting the whole time,
  independent of the run; samples are tagged rather than gated.
- **PMOD JD pin 1 (V27) is the nerve-phantom output** (`nerve_pulse`): high for
  1 ms starting 0.5 ms after each TIS onset, once per 200 ms stimulation, only
  while a run is active. It drives an external switched-resistor board that
  emulates a nerve impedance change.
...
|---|---|---|---|
| `addr_gen:1.0` | `addr_gen_1_0` | Verilog | AXI-Lite; four 32-bit DDS phase accumulators feeding the 4 sine-LUT BRAMs; `dds_sync` parks the A/B (TIS carrier) phase at 0 between bursts |
...
| `IP_1:1.0` | `IP_1_1_0` | Verilog | AXI-Lite; **TIS/EIT master sequencer** — 100 MHz time base drives the 200 ms stimulation cycle, TIS burst window, nerve-phantom pulse, BTNU-started bounded runs ([README](ip_repo/IP_1_1_0/README.md)) |
...
- An ILA (39 probes, 16k deep) is instantiated in the BD for bring-up; `probe36..38`
  are `tis_on`/`run_active`/`nerve_pulse`. Remove it
...
| IP interface change (new ports on an existing IP) | `ip_repo/<ip>/component.xml` via `scripts/repackage_ip.tcl` | No — headless, see below |
| Brand-new IP | `ip_repo/<ip>/` via IP Packager (writes back to the repo) | Strongly recommended |
...
2. Set the *IP location* to `TIS_EIT_V3/ip_repo/<YourIP_1_0>` so it is committed
...
```tcl
write_bd_tcl -force -include_layout E:/Vivado_Genesys2_main/TIS_EIT_V3/bd/ltc2500_bd.tcl
```
...
> project actually sources — `TIS_EIT_V3/bd/ltc2500_bd.tcl`, per
...
> `TIS_EIT_V3/bd/` and `git checkout -- TIS_EIT_V1/bd/ltc2500_bd.tcl`.

Then **before committing**, run `git diff TIS_EIT_V3/bd/ltc2500_bd.tcl` and
...
- Create `sim/tb_<name>.sv`, then either re-run the recreate script, or just
  ...
  `add_files -fileset sim_1 <repo>/TIS_EIT_V3/sim/tb_<name>.sv`
...
1. `work/` is disposable output: never commit it, never edit files under it
   (enforced by `TIS_EIT_V3/.gitignore`; the repo-root `.gitignore` filters
...
3. After every `write_bd_tcl` re-export: `git diff` and re-apply the three
   ...
   Prefer hand-editing `bd/ltc2500_bd.tcl` so no re-export happens at all.
3a. Added or removed a **port** on a packaged IP? Run
   `scripts/repackage_ip.tcl -tclargs <ip_folder>` before `recreate_project.tcl`,
   or the BD still sees the old interface and cannot connect the new pin. The
   script sets the IP's top module explicitly (read from `component.xml`) — left
   to auto-inference Vivado picks the inner `*_S00_AXI` module and silently
   strips the whole `S00_AXI` interface out of the component.
...
TIS_EIT_V3 is a real-time **EIT (electrical impedance tomography) acquisition
...
- **`addr_gen_1_0`** — four independent 32-bit DDS phase accumulators (A–D).
  ...
  FSM; `acc[31:16] + offset` addresses its 64k×16 sine BRAM (`coe/`). Using the
  **top** half of the accumulator makes this a true 32-bit DDS (resolution
  `f_update/2^32` ~= 0.00005 Hz), which is what lets F1/F2 hit 2000.000/2020.000 Hz
  for an exact 20 Hz TIS beat; indexing with `acc[15:0]` gave only 3.05 Hz steps.
  Channels A/B are held at phase 0 while `dds_sync` (= `IP_1/dds_hold`) is high.
  Channel **A = F1 (2000 Hz)** and **B = F2 (2020 Hz)** are the TIS carrier pair;
  C is an independent always-on tone and D is spare — neither is burst-gated.
- **`FSM_1_0`** — AD5686R quad-DAC SPI driver ([full README](ip_repo/FSM_1_0/README.md)):
  ...
  sample rate. SCLK divider, update period and per-channel Q12 gains are
  software-programmable. Its `tis_on` input forces the **A/B** gain to zero
  between bursts (mid-scale output, no current); software cannot do this because
  gain `0` means *unity* in that register.
- **`IP_1_1_0`** — **TIS/EIT master sequencer**
  ([full README](ip_repo/IP_1_1_0/README.md)). A free-running 100 MHz counter
  divides time into 200 ms *stimulations*: mux switch at t=0, a 90 ms settling +
  **baseline** window, then a 100 ms F1/F2 burst (`tis_on`), then a 10 ms tail.
  It pulses `done_tick` per stimulation and `total_tick` per 8, emits the
  nerve-phantom pulse, parks the A/B DDS phase between bursts (`dds_hold`), and
  runs a bounded `frame_count`-frame measurement started by BTNU. Two scan modes
  select whether the sense channel is held for a whole stimulation (64
  stimulations/frame) or multiplexed within one (8/frame).
...
1. BTNU press → `IP_1` starts a run of `frame_count` frames (`run_active` high).
2. Each **stimulation** = 200 ms: `done_tick` switches the muxes, 90 ms settle +
   baseline, 100 ms F1/F2 burst (`tis_on`), 10 ms tail. The nerve-phantom pulse
   fires 0.5 ms into the burst for 1 ms.
3. 8 stimulations → `total_tick`. A **frame** = one full sweep of the 8 injection
   pairs: 64 stimulations in mode A, 8 in mode B (see the scan-mode table in
   [`IP_1`'s README](ip_repo/IP_1_1_0/README.md)).
4. `frame_count` frames → run ends, back to idle until the next press.
5. Independently and continuously, every `o_mclk` group = one LTC2500 sample →
   6 bytes (2-byte tag + 32-bit sample) → packet buffer; the sense mux moves on
   the `o_mclk` falling edge. The ADC is never gated by TIS — the tag byte's
   `TIS`/`RUN` bits let the host separate baseline from stimulation offline.
6. Every full buffer bank → one UDP frame on the wire.
